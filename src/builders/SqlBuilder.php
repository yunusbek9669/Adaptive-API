...
namespace app\components\traits\sqlMakers\builders;

use app\components\traits\sqlMakers\ASTValidator;
use app\components\traits\sqlMakers\public\CteConstants;
use app\components\traits\sqlMakers\public\CteRootRelation;
    ...
    protected function finalBuilder(array $params, string $dataTypeList, array $callbackList): array
    {
        if (!empty($this->select))
        {
            try {
                ...
                throw new \Exception('⛔️ '.preg_replace('/SQLSTATE\[\d+\]: /', '', BaseModel::modelErrorsToString($e)));
            }
...
//        prd($sql);
    ...
    protected function jsonBuilder(array|string &$dataForm, string $dataTypeList, array $callbackList): array
    {
        ...
        $iteration = 0;
        $data_types = [];
        $params = array_merge(['status' => BaseModel::STATUS_ACTIVE], $this->params);
        ...
        return $this->finalBuilder($params, $dataTypeList, $callbackList);
    }
